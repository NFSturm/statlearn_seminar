...
Der niedrigste Wert für den RMSE wird in der Dekade um 270 aggregierte Bäume erreicht. Dann liegt der Fehler bei 433. Die Evaluation der Anzahl der verwendeten Bäume lässt sich gut grafisch veranschaulichen. 

```{r}
ggplot(tree_prep, aes(nr_tree, RMSE)) + geom_line(col = "#7fa9c1", size = 1) + theme_classic() + labs(title = "RMSE-Maß für Bootstrapping") + xlab("Anzahl der Bootstrap-Replikationen") + ylab("Cross-Validation RMSE")
```
...
Das beste Modell erreicht einen RMSE-Wert von etwa 391. 
...
Für mtry = 7 erreicht der Random Forest den geringsten Kreuzvalidierungsfehler von 373. 
...
```{r}
ggplot(cv_select, aes(x = mtry, y = mean.rmse)) + geom_line(col = "#7fa9c1", size = 1) + geom_point(col = "#7fa9c1", size = 3) + theme_classic() + labs(title = "Random Forest RMSE", subtitle = "Auf Basis 15 diskreter Werte für Mtry") + xlab("Mtry") + ylab("Cross-validated RMSE") + scale_x_continuous(breaks = seq(1, 30, 2))
```