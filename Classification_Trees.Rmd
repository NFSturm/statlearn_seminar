...
```{r}
tree_pruned <- prune.rpart(tree_churn, cp = 0.024)
```
...
Für einen Aggregationswert von 26 hat das Modell die höchste Genauigkeit für den Test-Datensatz mit 86.1%. 
...
Die Vorhersagegenauigkeit des besten Modells beträgt etwa 83%. 
...
```{r}
grid <- expand.grid(mtry = seq(1,29, 1), min.node.size = 5, splitrule = "gini")
control <- trainControl(method="cv", number = 10)
ranger_model <- train(Attrition ~., data = train2, method = "ranger", importance = "impurity", tuneGrid = grid, trControl = control)

ggplot(ranger_model) + geom_point(col = "#7fa9c1") + geom_line(col = "#7fa9c1") + theme_classic() + xlab("Anzahl von Mtry") + scale_x_continuous(breaks = c(1,5,10,15,20,25,30))

```

Die beste Performance ergibt sich für mtry = 26. 

```{r}
ranger_model_best <- ranger(Attrition~., data = train, mtry = 26)
ranger_preds <- predict(ranger_model_best, test)
predictions <- customRound(ranger_preds$predictions)
Metrics::accuracy(actual, predictions)
```

Es ergibt sich eine Genauigkeit von ca. 84%. 